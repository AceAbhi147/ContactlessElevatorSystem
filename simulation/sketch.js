...
class Stats {
    ...
    chargeRider(p, tripTime) {
        ...
        const rideCost =
            this.normalRideCost - p.map(penaltyTime, 0, 300, 0, this.normalRideCost)
...
class Controls {
    ...
    createKnobs(passengerLoadTypes) {
        ...
        projection.changed(
            () => (settings.projectionType = projection.elt.selectedIndex)
        )
        ...
        controlMode.changed(
            () => (settings.controlMode = controlMode.elt.selectedIndex)
        )
...
class Building {
        ...
        for (let floor = 1; floor <= this.settings.numFloors; ++floor) {
            ...
            p.pushed(() => {
                ...
                p.box(
                    ...
                    floorHeight,
                    floor === 1
                        ? this.settings.geom.floorDepthGround
                        : this.settings.geom.floorDepthOthers
                )
            })
            this.cars.forEach(car => {
                p.pushed(() => {
                    ...
                    p.translate(
                        car.carCenterX(),
                        floorY + gc.y + indHeight / 2,
                        this.settings.geom.carCenterZ + gc.z / 2
                    )
                    p.noStroke()
                    const carReady =
                        floorY === car.y &&
                        (car.state === CarState.Opening || car.state === CarState.Open)
                    if (carReady) {
    ...
    drawUpDownIndicator(p, indHeight, goingUp) {
        ...
        p.noStroke()
        p.fill("green")
        goingUp ? p.triangle(0, 5, -4, -4, 4, -4) : p.triangle(0, -4, -4, 5, 4, 5)
    }
}
...
class Car {
    constructor(p, settings, stats, carNumber) {
        ...
        const carsGroupWidth =
            settings.numCars * gc.x + (settings.numCars - 1) * interCarSpacing
        ...
        this.pan =
            settings.numCars === 1
                ? 0
                : p.map(carNumber, 1, settings.numCars, -0.8, 0.8)
    ...
    drawRails() {
        ...
        ;[-halfCarWidth, halfCarWidth].forEach(xOff => {
            ;[-halfCarDepth, halfCarDepth].forEach(zOff => {
                p.pushed(() => {
                    p.translate(
                        this.carCenterX() + xOff,
                        p.height / 2,
                        this.settings.geom.carCenterZ + zOff
                    )
                    p.box(2, p.height, 1)
                })
            })
    ...
    update() {
        ...
        switch (this.state) {
            ...
            case CarState.Opening:
                this.doorOpenFraction = p.constrain(
                    (this.nowSecs() - this.doorOpStarted) /
                    this.settings.doorMovementSecs,
                    0,
                    1
                )
    ...
    idle(p) {
        if (this.destFloors.length) {
            ...
            this.stats.addMovementCosts(
                Math.abs(p.floorFromY(this.y) - nextDest),
                this.settings.elevSpeed
            )
    ...
    move(p) {
        ...
        const ΔySinceLastMove = Math.min(
            absTravelLeft,
            this.speed * ΔtSinceLastMove
        )
    ...
    accelerating() {
        return (
            Math.abs(this.y - this.startY) < this.accelDistance &&
            this.speed < this.maxSpeed
        )
    }

    removeCurrentFloorFromDest() {
        this.destFloors = this.destFloors.filter(
            f => this.p.yFromFloor(f) !== this.y
        )
    }
...
class Dispatcher {
    ...
    requestCar(floor, goingUp) {
        if (
            !this.carCallQueue.find(
                request => request.floor === floor && request.goingUp === goingUp
            )
        ) {
        ...
        if (this.settings.controlMode === 0 /* Auto */) {
            ...
            if (request) {
                ...
                const idleCars = activeCars.filter(
                    car => car.state === CarState.Idle && car.goingUp === request.goingUp
                )
                const dist = car => Math.abs(car.y - floorY)
                const closest = cars =>
                    cars.reduce(
                        (a, b) => (a && b ? (dist(a) > dist(b) ? b : a) : b),
                        undefined
                    )
                ...
                if (closestIdleActiveCar) {
                    ...
                    if (closestActiveCar) closestActiveCar.goTo(request.floor)
...
class Rider {
    constructor(p, settings, startFloor, destFloor, dispatcher, stats, talker) {
        ...
        this.pos = p.createVector(
            enterX,
            p.yFromFloor(startFloor),
            this.randomFloorZ()
        )
        const waitX =
            enterX + travelDirection * p.randomGaussian(p.width / 3, p.width / 4)
    ...
    setBodyAttributes() {
        ...
        const normalWaistDiam = 0.9 / Math.PI // d = circumference / π
    ...
    update() {
        ...
        switch (this.state) {
            ...
            case RiderState.Boarding:
                const canceled = this.followPath(
                    this.boardingPath,
                    RiderState.Riding,
                    () => {
    ...
    requestCar() {
        this.dispatcher.requestCar(
            this.startFloor,
            this.destFloor > this.startFloor
        )
    }
    ...
    outsideDoorPos(openCar) {
        return this.p.createVector(
            openCar.carCenterX() + this.fuzz(2),
            this.pos.y,
            openCar.settings.geom.carCenterZ + this.carGeom.z + this.fuzz(2)
        )
    }
    ...
    ride() {
        ...
        if (
            car.state === CarState.Open &&
            car.y === this.p.yFromFloor(this.destFloor)
        ) {
    ...
    setBoardingPath(car) {
        ...
        const insideCar = this.p.createVector(
            car.carCenterX() + this.fuzz(cg.x * 0.4),
            this.pos.y,
    ...
    setExitingPath(car) {
        const p = this.p
        const nearDoorInsideCar = p.createVector(
            car.carCenterX() + this.fuzz(2),
            this.pos.y,
            car.settings.geom.carCenterZ + this.carGeom.z / 2 - 5 + this.fuzz(2)
        )
        const outsideDoor = this.outsideDoorPos(car)
        const exitPoint = p.createVector(
            p.width / 2 - (this.p.random([-1, 1]) * p.width) / 2,
            this.pos.y,
            this.randomFloorZ()
        )
    ...
    moveToward(dest) {
        ...
        const stepMagnitude = Math.min(
            distToDest,
            this.movementPerMs * millisSinceLastStep
        )
...
class Talker {
    ...
    whenLoaded(loaded = () => {}) {
        ...
        function populateVoiceList() {
            if (
                ...
                talker.voices !== undefined
            ) {
                return
            }

            const unwantedVoices = new Set(
                "Alex Daniel Fred Jorge Victoria Zosia".split(" ")
            )
            const allVoices = speechSynthesis.getVoices()
            if (allVoices.length) {
                talker.voices = talker.speech
                    .getVoices()
                    .filter(v => !unwantedVoices.has(v.name))
                talker.englishVoices = talker.voices.filter(v =>
    ...
    speak(message, voiceName) {
        if (
            new Date().getTime() > this.nextSpeechAllowedTime &&
            this.voices.length &&
            !this.speech.speaking &&
            this.settings.speakersType > 0
        ) {
